...
const heartRateZoneConfig = [
  ...
  { name: 'Z3', lower: 0.90, upper: 0.94, color: '#eab308' }, // Yellow
...
export const Dashboard: React.FC<DashboardProps> = ({ data, workoutHistory, workout, settings }) => {
  ...
  const chartData = useMemo(() => {
    ...
    const profileData: { time: number; targetPace: number; targetIncline: number; pace: number | null; incline: number | null; heartRate: number | null; }[] = [];
  ...
  const paceDomain = useMemo(() => {
    ...
    chartData.forEach(d => {
      const paces = [d.targetPace, d.pace].filter(p => p !== null && isFinite(p)) as number[];
    ...
    if (!isFinite(minPace)) return [180, 600];
  ...
  return (
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg shadow-lg p-6 space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={<SpeedIcon className="w-6 h-6 text-cyan-400" />} label="Pace" value={formatPaceSecs(speedKmhToPaceSecs(data.speed))} unit="min/km" />
      ...
      <div className="h-80 w-full">
        {workout ? (
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }} barGap={0} barCategoryGap={0}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
              <XAxis dataKey="time" stroke="#9ca3af" tickFormatter={(time) => `${Math.floor(time/60)}m`}/>
              <YAxis 
                yAxisId="left" 
                stroke="#38bdf8" 
                label={{ value: 'Pace (min/km)', angle: -90, position: 'center', dx: -25, fill: '#38bdf8' }}
              ...
              <YAxis 
                ...
                orientation="right" 
                stroke="#f87171" 
                label={{ value: 'Heart Rate (bpm)', angle: 90, position: 'center', dx: 25, fill: '#f87171' }} 
              ...
              {zones.map(zone => (
                <ReferenceArea 
                  ...
                  fill={zone.color} 
                  fillOpacity={0.1}
                  label={{ value: zone.name, position: 'insideTopRight', fill: zone.color, fontSize: 10, dy: 12, dx: -10 }}
                />
              ))}

              {/* Workout Profile Bars */}
              <Bar yAxisId="left" dataKey="targetPace" fill="#38bdf8" fillOpacity={0.4} name="Target Pace" isAnimationActive={false}/>
              <Bar yAxisId="incline-hidden" dataKey="targetIncline" fill="#4ade80" fillOpacity={0.4} name="Target Incline" isAnimationActive={false}/>

              {/* Live Data Lines */}
              <Line yAxisId="left" type="monotone" dataKey="pace" stroke="#0ea5e9" strokeWidth={3} name="Actual Pace" dot={false} connectNulls={false} isAnimationActive={false} />
              <Line yAxisId="incline-hidden" type="monotone" dataKey="incline" stroke="#22c55e" strokeWidth={3} name="Actual Incline" dot={false} connectNulls={false} isAnimationActive={false} />
              <Line yAxisId="right" type="monotone" dataKey="heartRate" stroke="#f87171" strokeWidth={2} name="Heart Rate" dot={false} connectNulls={false} isAnimationActive={false} />
            </ComposedChart>
          </ResponsiveContainer>
        ) : (
