...
const WorkoutStepView: React.FC<{
	step: WorkoutStep;
	title: string;
	isActive?: boolean;
}> = ({ step, title, isActive = false }) => (
	<div
		className={`p-3 rounded-md ${isActive ? "bg-cyan-900/50 ring-2 ring-cyan-500" : "bg-gray-700"}`}
	>
		<p
			className={`font-semibold ${isActive ? "text-cyan-400" : "text-gray-400"}`}
		>
			{title}
		</p>
			...
			<div className="flex items-center space-x-3 text-sm">
				{step.speed !== undefined && (
					<span className="flex items-center">
						<SpeedIcon className="w-4 h-4 mr-1 text-cyan-400" />{" "}
						{formatSpeedToPace(step.speed)} min/km
					</span>
				)}
				{step.incline !== undefined && (
					<span className="flex items-center">
						<InclineIcon className="w-4 h-4 mr-1 text-green-400" />{" "}
						{step.incline.toFixed(1)}%
					</span>
				)}
			</div>
		</div>
...
export const WorkoutPlayer: React.FC<WorkoutPlayerProps> = ({
	...
	const stepProgress = currentStep
		? (timeInStep / currentStep.duration) * 100
		: 0;
	const totalProgress =
		totalDuration > 0 ? (totalTime / totalDuration) * 100 : 0;
	...
	return (
		<div className="space-y-4">
			<div className="flex justify-between items-center">
				<h2 className="text-xl font-bold truncate" title={workout.name}>
					{workout.name}
				</h2>
				<button
					type="button"
			...
			<div className="flex justify-center items-center space-x-4 pt-4">
				<button
					...
					title={
						!isConnected
							? "Connect to treadmill to stop workout"
							: "Stop Workout"
					}
				>
				...
				<button
					...
					title={
						!isConnected && isPaused
							? "Connect to treadmill to start workout"
							: isPaused
								? "Play Workout"
								: "Pause Workout"
					}
