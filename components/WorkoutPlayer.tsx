...
import { InclineIcon, PauseIcon, PlayIcon, ResetIcon, SpeedIcon, StopIcon } from './Icons'
      ...
      <div className="flex items-center space-x-3 text-sm">
        {step.speed !== undefined && (
          <span className="flex items-center">
            <SpeedIcon className="w-4 h-4 mr-1 text-cyan-400" /> {formatSpeedToPace(step.speed)} min/km
          </span>
        )}
        {step.incline !== undefined && (
          <span className="flex items-center">
            <InclineIcon className="w-4 h-4 mr-1 text-green-400" /> {step.incline.toFixed(1)}%
          </span>
        )}
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button type="button" onClick={onReset} className="p-1 text-gray-400 hover:text-white transition">
      ...
      <div className="space-y-4 pt-2">
        ...
        <div>
          <div className="flex justify-between text-xs mb-1 text-gray-400">
            ...
            <span>
              {formatTime(timeInStep)} / {formatTime(currentStep?.duration ?? 0)}
            </span>
          </div>
