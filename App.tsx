...
function App() {
	...
	const onTreadmillData = useCallback(
		(data: { speed: number; incline: number; heartRate?: number }) => {
	...
	const handleStartWorkout = () => {
		if (ftms.connectionStatus === ConnectionStatus.CONNECTED) {
			...
			alert(
				`Please connect to a treadmill first${isTestMode ? " (in test mode)" : ""}.`,
			);
		}
	};
		...
		<div className="flex flex-col h-full">
			...
			<div className="flex-shrink-0">
				<ManualControls
					...
					disabled={
						ftms.connectionStatus !== ConnectionStatus.CONNECTED ||
						workoutControl.isActive
					}
				/>
			</div>
		</div>
								...
								},
								{
									...
									content: (
										<Settings
											...
											onThresholdPaceChange={(pace) =>
												setSettings((s) => ({ ...s, thresholdPace: pace }))
											}
											thresholdHr={settings.thresholdHr}
											onThresholdHrChange={(hr) =>
												setSettings((s) => ({ ...s, thresholdHr: hr }))
											}
										/>
									),
								},
